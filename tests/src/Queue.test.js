...
describe('Test allQueues file', () => {
    ...
    test('should test failedJobsEnqueue', () => {
        const jobToBeCreated1 = {
            id: 11,
            name: 'testJob1',
            param: {},
        ...
        queue.failedJobEnqueue(job2);
        queue.failedJobEnqueue(job3);
        queue.failedJobsEnqueue();
        expect(dbAdapter.items).toContain(job1);
        expect(dbAdapter.items).toContain(job2);
        expect(dbAdapter.items).toContain(job3);
        expect(dbAdapter.failedItems.length).toBe(0);
    });
});
